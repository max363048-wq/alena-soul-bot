# safety.py — Модуль фильтрации: грубость (блокировка) и опасные темы (мягкий уход)
...
# === 2. Опасные темы (не блокируются, но требуют мягкого ухода) ===
SENSITIVE_TOPICS = [
...
def is_profanity(text: str) -> bool:
    """Проверка на мат и оскорбления (блокируем)"""
    if not text:
        return False
    return bool(PROFANITY_PATTERN.search(text))

def is_sensitive_topic(text: str) -> Tuple[bool, str]:
    """Проверка на опасные темы (не блокируем, но нужен мягкий уход). Возвращает (да/нет, найденная тема)"""
    if not text:
        return False, ""
...
# === 3. Свидания (с подсчётом попыток) - без изменений ===
DATING_PHRASES = [
...
# Инструкция для опасных тем (чтобы Алёна сама ответила)
