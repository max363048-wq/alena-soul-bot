# main.py — финальный, без ложных срабатываний
import os
import telebot
import re
import random
...
def handle_message(message: telebot.types.Message) -> None:
    ...
    # --- Основной показ фото (урезан опасные фразы) ---
    if re.search(r'(покажи\s+фото|покажи\s+мне\s+фото|покажи\s+свои\s+фото|покажи\s+фотки|покажешь\s+фото|своё\s+фото|свое\s+фото|фотоальбом|покажи\s+себя|покажи\s+где\s+ты|покажи\s+картинку|покажи\s+изображение|есть\s+ли\s+у\s+тебя\s+фото|посмотреть\s+твои\s+фото|покажи\s+любимое\s+фото|какое\s+твое\s+любимое\s+фото|покажи\s+другое\s+фото|ещё\s+фото|фотки\s+где\s+ты|какие\s+фото\s+у\s+тебя\s+еще\s+есть|какие\s+фото\s+ещё\s+есть|какие\s+у\s+тебя\s+ещё\s+фото|какие\s+ещё\s+фото|какие\s+фото\s+еще|какие\s+еще\s+фото|какие\s+у\s+тебя\s+есть\s+фото)', user_text, re.IGNORECASE):
        user_pending_photo_offer[user_id] = False
