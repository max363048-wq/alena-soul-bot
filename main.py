...
user_last_photo: Dict[int, str] = {}          # последнее показанное фото
user_shown_photos: Dict[int, set] = {}        # показанные фото в текущей сессии
user_no_photos: Dict[int, bool] = {}          # запоминает, сказал ли пользователь, что у него нет своих фото
...
def reset_user(user_id: int) -> None:
    ...
    user_shown_photos.pop(user_id, None)
...
def get_random_photo_excluding(user_id: int) -> Optional[str]:
    """Возвращает случайное фото из папки, стараясь не показывать то же, что и в прошлый раз, и учитывая уже показанные в сессии."""
    available = get_photo_list()
    if not available:
        return None
    shown = user_shown_photos.get(user_id, set())
    # Если все фото уже показаны, сбрасываем счётчик для этой сессии (но можно оставить как есть – тогда будет повтор)
    if len(shown) >= len(available):
        # Сбрасываем, чтобы начать заново
        user_shown_photos[user_id] = set()
        shown = set()
    # Ищем фото, которое ещё не показывали
    candidates = [p for p in available if p not in shown]
    if not candidates:
        candidates = available  # на всякий случай
    chosen = random.choice(candidates)
    user_shown_photos[user_id] = shown.union({chosen})
...
if __name__ == '__main__':
    print('✅ Алёна стабильная — упрощённый показ фото, запоминание показанных, без лишних "Привет"')
