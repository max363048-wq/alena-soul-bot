# main.py — Финальная версия с диагностикой Groq
...
def weather_cmd(message: telebot.types.Message):
    # полная реализация (можно оставить заглушку, но лучше полную)
    pass
...
def handle_message(message: telebot.types.Message):
    ...
    if re.search(r'(расскажи|поделись|придумай|дай|хочешь рассказать).*?(историю|рассказ|случай|байку|истории)\b|какую(?:-?нибудь|-?то)?\s+историю', user_text, re.IGNORECASE):
        ...
        print(f"[LLM] Отправка запроса в Groq: {user_text[:100]}")
        reply = None
        for attempt in range(2):
            try:
                ...
                print(f"[LLM] Успешный ответ: {reply[:100]}")
                break
            except Exception as e:
...
if __name__ == '__main__':
    print('✅ Алёна — диагностическая версия')
    try:
