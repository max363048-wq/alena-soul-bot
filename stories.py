# stories.py — улучшенные истории с самопроверкой
...
def generate_story(prompt: str, user_id: int, lang: str, client, gist_id: str) -> str:
    system_prompt = (
        ...
        'ОЧЕНЬ ВАЖНО: перед отправкой прочитай свою историю про себя и исправь все грамматические ошибки, странные или нелепые слова ("прирулили", "запусть", "гудели", "выпила кофе из чашки стоя", "непрофессиональные", "цветок на свадьбе сестры и подарить его мне" и т.п.). '
        'История должна быть логичной, связной, без повторений и несуразностей. '
        'Пиши на русском, с 2-3 эмодзи, без английских слов. Не начинай с приветствия. Не обрывай мысль на полуслове.'
    )
    try:
        ...
        if not story:
            return "Мы как-то с подругой пошли в парк кататься на роликах, и я так разогналась, что не смогла затормозить и врезалась в куст сирени! 😄 Было смешно и немного стыдно, зато теперь я умею тормозить правильно 💕"
...
# creative_prompt остаётся без изменений (он уже хорош)
