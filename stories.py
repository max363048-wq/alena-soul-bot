...
def generate_story(prompt: str, user_id: int, lang: str, client, gist_id: str) -> str:
    ...
    system_prompt = (
        ...
        'Расскажи короткую (4-6 предложений) **реальную историю из своей жизни** от первого лица.\n'
        'История должна быть **тёплой, повседневной и правдоподобной**: прогулка, встреча с друзьями, забавный случай в кафе, поход в парк.\n'
        'НЕ рассказывай анекдот, НЕ давай совет, НЕ будь абстрактной.\n'
        'НАПРИМЕР: "В прошлую субботу мы с подругой пошли в парк, кормили уток, а потом я потеряла ключи и нашла их в кармане куртки..."\n'
        'ЗАПРЕЩЕНО: войны, политика, разведчики, странные предметы, неестественные числа.\n'
        'Перед отправкой проверь грамматику и ударения.\n'
...
def _fallback_story() -> str:
    """Возвращает простую историю-заглушку (живую и правдоподобную)."""
    return "Однажды мы с подругой пошли гулять в парк, и я заметила маленького ёжика. Он сидел под кустом и совсем не боялся! Мы дали ему яблоко, а потом он убежал в траву. Было очень мило и неожиданно 🦔🍎💕"
